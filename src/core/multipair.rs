...
mod test {
    ...
    fn borrow_test() {
        ...
        assert_eq!(
            <MultiPair<usize, usize> as Borrow<usize>>::borrow(&pair),
            &1usize
        );
    }
    ...
    fn range_bounds() {
        ...
        let start_1 = vec
            ...
            .unwrap();
        let end_1 = vec
            .rank(Excluded(&MultiPair::with_supremum(1)), true)
            .unwrap();
        ...
        let end_2 = vec
            .rank(Excluded(&MultiPair::with_supremum(2)), true)
            .unwrap();
        ...
        let start_3 = vec
            .rank(Included(&MultiPair::with_infimum(3)), true)
            .unwrap()
            + 1;
        let end_3 = vec
            .rank(Excluded(&MultiPair::with_supremum(3)), true)
            .unwrap();
        ...
        let start_4 = vec
            .rank(Included(&MultiPair::with_infimum(4)), true)
            .unwrap()
            + 1;
        let end_4 = vec
            .rank(Excluded(&MultiPair::with_supremum(4)), true)
            .unwrap();
